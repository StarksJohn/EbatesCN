 ...
 */
export function checkAvailableTokenExpires() {
    return new Promise(
        (resolve, reject) => {
            ...
            if (tokenSchema.isExpires()) {//判断 可用的token是否 过期
               ...
               return tokenSchema.refreshToken();
            } else {
                Log.log('TokenAPI checkAvailableTokenExpires 当前可用的 '+tokenSchema.name+' 未过期,可开始调 接口 ')
                resolve();
            }
        }
    );
